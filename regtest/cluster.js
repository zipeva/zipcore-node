...
describe('Zip Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.zipcore/data/zipd')
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/zip.conf'),
      rpcuser: 'zip',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/zip.conf'),
      rpcuser: 'zip',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/zip.conf'),
      rpcuser: 'zip',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 zipd daemons');
    this.timeout(200000);
  ...
  it('step 1: will connect to three zipd daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = zipcore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.zipd.once('tip', function(height) {
