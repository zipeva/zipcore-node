...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for zipd is necessary for this upgrade with the "reindex=1" zip.conf option. \n' +
                 'There are changes necessary in both zip.conf and zipcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/zipeva/zipcore-node/blob/zipd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          zipd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/zipd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed zipcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './zipcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in zipcore-node services directory
    ...
    if (servicePackage.zipcoreNode) {
      serviceModule = service.name + '/' + servicePackage.zipcoreNode;
    }
 ...
 * @param {Node} node - The Zipcore Node instance
 */
