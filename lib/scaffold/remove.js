...
/**
 * Will remove a service from zipcore-node.json
...
/**
 * Will remove the Node.js service and from the zipcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The zipcore-node configuration directory
...
function remove(options, done) {
  ...
  var zipcoreConfigPath = path.resolve(configPath, 'zipcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(zipcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a zipcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to zipcore-node.json
        removeConfig(zipcoreConfigPath, service, next);
      });
    }, done
  );
}
