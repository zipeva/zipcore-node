...
var Proposal = zipcore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to zipd in Node.js. Manages starting and
 * stopping zipd as a child process for application support, as well as connecting
 * to multiple zipd processes for server infrastructure. Results are cached in an
...
util.inherits(Zip, Service);

Zip.dependencies = [];

Zip.DEFAULT_MAX_TXIDS = 1000;
Zip.DEFAULT_MAX_HISTORY = 50;
Zip.DEFAULT_MAX_UTXO = 1000;
Zip.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Zip.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Zip.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Zip.DEFAULT_SPAWN_RESTART_TIME = 5000;
Zip.DEFAULT_SPAWN_STOP_TIME = 10000;
Zip.DEFAULT_TRY_ALL_INTERVAL = 1000;
Zip.DEFAULT_REINDEX_INTERVAL = 10000;
Zip.DEFAULT_START_RETRY_INTERVAL = 5000;
Zip.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Zip.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Zip.DEFAULT_INSTANTSEND_FEE = 10000;
Zip.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Zip.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Zip.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Zip.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Zip.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Zip.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Zip.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Zip.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Zip.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Zip.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Zip.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Zip.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Zip.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Zip.prototype._initCaches = function() {
...
Zip.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Zip.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Zip.prototype.getPublishEvents = function() {
  return [
    {
...
Zip.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'zipd/' + name, 'total:', this.subscriptions[name].length);
};

Zip.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'zipd/' + name, 'total:', this.subscriptions[name].length);
};

Zip.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (zipcore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'zipd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Zip.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'zipd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Zip.prototype.unsubscribeAddressAll = function(emitter) {
...
Zip.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Zip.DEFAULT_CONFIG_SETTINGS;
...
Zip.prototype._parseZipConf = function(configPath) {
  var options = {};
...
Zip.prototype._expandRelativeDatadir = function() {
...
Zip.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in zipd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in zipd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in zipd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './zip.conf');

  log.info('Using Zip config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseZipConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseZipConf(networkConfigPath));
  }
...
Zip.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of zipcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of zipcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of zipcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in zip.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that zipd is undergoing a reindex. ' +
             ...
             'of zipcore-node services will start.');
    node._reindex = true;
  }
};

Zip.prototype._resetCaches = function() {
 ...
 * @private
 */
Zip.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Zip.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Zip.prototype._initChain = function(callback) {
  var self = this;
...
Zip.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 28081
  };
  if (this.node.network === zipcore.Networks.testnet) {
    networkOptions.rpcport = 38081;
  }
  return networkOptions;
};

Zip.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === zipcore.Networks.testnet) {
    networkPath = 'testnet3/zip.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/zip.conf';
    }
  }
  return networkPath;
};

Zip.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === zipcore.Networks.testnet) {
...
Zip.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('zipd/hashblock', message.toString('hex'));
    }
  }

};

Zip.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Zip.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Zip Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Zip.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Zip.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('zipd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Zip.prototype._zmqTransactionHandler = function(node, message) {
  ...
  var self = this;
  var hash = zipcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('zipd/rawtransaction', message.toString('hex'));
    }

    var tx = zipcore.Transaction();
    tx.fromString(message);
    var txid = zipcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Zip.prototype._zmqTransactionLockHandler = function (node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('zipd/transactionlock', message.toString('hex'));
    }

  }
};

Zip.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Zip.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Zip.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Zip.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Zip.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Zip Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Zip.DEFAULT_REINDEX_INTERVAL);

};

Zip.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Zip.prototype._stopSpawnedZip = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/zipd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned zip process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean zip process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Zip.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedZip(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting zip process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Zip process unexpectedly exited with code:', code);
        log.warn('Restarting zip child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Zip process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn zipd.'));
      }
...
Zip.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to zipd.'));
    }
 ...
 */
Zip.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Zip configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Zip.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Zip.prototype.syncPercentage = function(callback) {
  var self = this;
...
Zip.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, zipcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Zip.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = zipcore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Zip.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Zip.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Zip.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Zip.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in zipcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Zip.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Zip.prototype._getConfirmationsDetail = function(transaction) {
...
Zip.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Zip.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Zip.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A zip transaction id
 * @param {Function} callback
 */
Zip.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Zip.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof zipcore.Address) {
...
Zip.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Zip.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Zip.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Zip.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Zip.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Zipcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Zip.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = zipcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Zip.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Zip.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Zip.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Zip.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Zip.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Zipcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Zip.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Zip.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Zip.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Zip.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Zip.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Zip.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Zip.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Zip.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Zip.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Zip.prototype.govCount = function (callback) {
  var self = this;
...
Zip.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Zip.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Zip.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Zip.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Zip.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.getBestChainLock = function (callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.getInfo = function(callback) {
  var self = this;
...
Zip.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Zip.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('zipd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('zipd process did not exit'));
      }
